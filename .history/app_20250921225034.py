...
class VoiceBot:
    ...
    def listen_with_timeout(self, max_duration=10):
        """Record audio with a maximum duration but stop when silence is detected."""
        try:
            sample_rate = 44100
            channels = 1
            
            st.info(f"🎤 Recording... (max {max_duration} seconds, will auto-stop on silence)")
            
            # Record audio for the specified duration
            audio_data = sd.rec(int(max_duration * sample_rate), 
                              samplerate=sample_rate, 
                              channels=channels, 
                              dtype=np.float32)
            sd.wait()  # Wait until recording is finished
        ...
        with voice_col:
            ...
            if not st.session_state.is_recording:
                if st.button("🎤 Start", key="start_rec", use_container_width=True, help="Start voice recording"):
                    ...
                    success = st.session_state.voice_bot.start_continuous_recording()
                    if not success:
                        st.session_state.is_recording = False
                    st.rerun()
            else:
                if st.button("⏹️ Stop", key="stop_rec", use_container_width=True, help="Stop recording and process"):
                    ...
                    with st.spinner("🎤 Processing your voice..."):
                        question = st.session_state.voice_bot.stop_continuous_recording()
